...
### MAP FUNCTIONS ###
plot_map <- function(dataset, input_year, input_week, EPA){
  # Plot Main Choropleth
  if(EPA == TRUE){
    ggplot(dataset %>% filter(date == input_week),
           aes(long, lat)) + 
      ...
      coord_quickmap() + 
      theme_void() + 
      labs(title = "PM 2.5 Exposure (Per Capita)", fill = "PM 2.5 Exposure")
  }
  else{
...
compare_bars <- function(state, year){
  ...
  ggplot(na.omit(current) %>% group_by(state_name) %>% summarise(total = sum(PM25_pop_pred)), aes(y = fct_reorder(state_name,total), x =  total)) + 
    geom_col(fill = "#355070") + 
    geom_col(data = na.omit(current) %>% filter(state_name == state) 
...
server <- function(input, output, session) {
  ...
  observeEvent(input$year, {
    if(input$year==2016){current <- data2016}
    if(input$year==2017){current <- data2017}
    if(input$year==2018){current <- data2018}
    if(input$year==2019){current <- data2019}
    updateSelectInput(session, "week", choices = c(unique(current$date)), selected=current$date[1])
    output$choropleth <- renderPlot({
      plot_map(current, input$year, input$week, input$showEPA)
    })
  })
